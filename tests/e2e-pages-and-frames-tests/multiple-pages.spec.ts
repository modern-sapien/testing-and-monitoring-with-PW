...
test("multiple tab flow", async ({ page }) => {
  await page.goto('https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe');
  
  // initial frame
  await page.frameLocator('iframe[title="MDN Web Docs Interactive Example"]').getByRole('button', { name: 'Reset' }).click();
  ...
  }).getByRole('button', { name: 'Zoom out' }).click();
});
